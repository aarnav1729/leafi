...
const AuctionRoom = ({ username, role }) => {
  ...
  useEffect(() => {
    const fetchRFQDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/rfq/${rfqId}`);
        ...
        if (role === "vendor") {
          ...
          const bidResponse = await axios.get(`http://localhost:8000/api/quotes/${rfqId}`);
  ...
  useEffect(() => {
    const fetchBids = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/quotes/${rfqId}`);
        if (role === "vendor") {
  ...
  const handlePlaceBid = async () => {
    ...
    if (!isNaN(price) && !isNaN(numberOfTrucks)) {
      try {
        ...
        const rfqResponse = await axios.get(`http://localhost:8000/api/rfq/${rfqId}`);
        ...
        if (vendorQuotes[rfqId]) {
          // Update existing bid
          await axios.put(
            `http://localhost:8000/api/quote/${vendorQuotes[rfqId]._id}`,
            {
              rfqId,
              vendorName,
              quote: price,
          ...
          // Create a new bid
          await axios.post("http://localhost:8000/api/quote", {
            rfqId,
            vendorName,
            quote: price,
            numberOfTrucks,
